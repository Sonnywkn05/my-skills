...
- **THEN** the output avoids adding explanations, rationale, or content that Claude already knows, defaulting to minimal necessary content
...
The `openspec/config.yaml` `rules.proposal` SHALL require every proposal to include a **Baseline Measurement** section that declares what Claude does today (without the skill or change) before any implementation begins.
...
- **THEN** it names specific queries or scenarios where the current behavior falls short, not vague statements like "Claude doesn't do this yet"
